...
import pymysql
import dbconnection  # keep as you had it


# ---------- utility functions ----------
def sort_cont(character_contours):
	"""
	Sort contours left-to-right by bounding box x-coordinate.
	"""
	i = 0
...
def segment_chars(plate_img, fixed_width):
	"""
	Extract Value channel, adaptive-threshold, connected-components,
	and return list of character images (BGR) from the plate.
	"""
	# Value channel from HSV
	...
	for label in np.unique(labels):
		...
		cnt_result = cv2.findContours(labelMask,
									  cv2.RETR_EXTERNAL,
									  cv2.CHAIN_APPROX_SIMPLE)
		# handle different OpenCV return signatures
		cnts = cnt_result[0] if len(cnt_result) == 2 else cnt_result[1]
	...
	contour_result = cv2.findContours(charCandidates,
									  cv2.RETR_EXTERNAL,
									  cv2.CHAIN_APPROX_SIMPLE)
...
class PlateFinder:
	...
	def extract_contours(self, after_preprocess):
		cont_result = cv2.findContours(after_preprocess,
									   mode=cv2.RETR_EXTERNAL,
									   method=cv2.CHAIN_APPROX_NONE)
		contours = cont_result[0] if len(cont_result) == 2 else cont_result[1]
		return contours
	...
	def clean_plate(self, plate):
		...
		thresh = cv2.adaptiveThreshold(gray,
									   ...
									   cv2.THRESH_BINARY,
									   11, 2)
		cont_result = cv2.findContours(thresh.copy(),
									   cv2.RETR_EXTERNAL,
...
class OCR:
	...
	def convert_tensor(self, image, image_size_output):
		"""takes an image and transform it into a tensor shaped for the model"""
		image = cv2.resize(image,
						   dsize=(image_size_output, image_size_output),
	...
	def label_image_list(self, list_images, image_size_output):
		plate = ""
		for img in list_images:
			if cv2.waitKey(25) & 0xFF == ord('q'):
				break
			tensor = self.convert_tensor(img, image_size_output)
...
def start_anpr(
        input_files,
        model_path=r"/Users/amarjeet/Desktop/binary_128_0.50_ver3.pb",
        label_path=r"/Users/amarjeet/Desktop/binary_128_0.50_labels_ver2.txt"
):
    findPlate = PlateFinder(minPlateArea=4100, maxPlateArea=15000)
    model = OCR(modelFile=model_path, labelFile=label_path)
        ...
        while cap.isOpened():
            ...
            if possible_plates is not None:
                for i, p in enumerate(possible_plates):
                    ...
                    print("Recognized:", recognized_plate)
                    ...
                    try:
                        ...
                        with connection.cursor() as cursor:
                            ...
                            connection.commit()
                            print("Inserted into DB:", recognized_plate)
...
if __name__ == "__main__":
    video_file = [r"/Users/amarjeet/Desktop/VIDEO-2025-11-16-19-36-55.mp4"]
